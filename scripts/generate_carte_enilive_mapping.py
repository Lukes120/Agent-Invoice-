"""
Generator idempotente di config/carte_enilive_mapping.py partendo da
input/carte_enilive.xlsx.

File sorgente atteso (foglio "CARTE ENILIVE" o "Foglio1"):
    | numero_carta | targa | nickname | classificazione_fiscale | stato |

Solo le righe con stato='ATTIVO' vengono incluse nel mapping.

Genera config/carte_enilive_mapping.py con:
  - CARTE_ENILIVE_BY_NUMERO: dict[numero_carta_str, info]
  - Helper: get_carta_enilive(numero), get_classificazione_carta_enilive(numero)

Uso:
    python scripts/generate_carte_enilive_mapping.py
...
DEFAULT_XLSX = PROJECT_ROOT / "input" / "carte_enilive.xlsx"
DEFAULT_OUTPUT = PROJECT_ROOT / "config" / "carte_enilive_mapping.py"
...
def normalize_numero_carta(raw):
    """Numero carta Enilive: testo grezzo, senza conversione numerica.
...
def parse_xlsx(xlsx_path):
    ...
    sheet_name = None
    for cand in ("CARTE ENILIVE", "Carte Enilive", "Foglio1", "Sheet1"):
...
def render_module(by_numero, sheet_name, xlsx_path, n_skipped, n_dup, n_cls_anomale):
    ...
    L = []
    L.append('"""')
    L.append('Mappa carte carburante Enilive -> classificazione fiscale veicolo (Ecotel).')
    L.append('')
    L.append('AUTO-GENERATO da scripts/generate_carte_enilive_mapping.py')
    ...
    L.append('    python scripts/generate_carte_enilive_mapping.py')
    L.append('"""')
    L.append('from typing import Optional, Dict')
    L.append('')
    L.append('# Costanti classificazione (allineate a carte_carburante_mapping/UTA)')
    ...
    L.append('')
    L.append('# Mappa per NUMERO CARTA ENILIVE -> info veicolo/classe')
    L.append('# Chiave: stringa numero_carta (RiferimentoAmministrazione XML)')
    L.append('CARTE_ENILIVE_BY_NUMERO: Dict[str, dict] = {')
    ...
    L.append('')
    L.append('')
    L.append('def get_carta_enilive(numero: str) -> Optional[dict]:')
    L.append('    """Ritorna info carta Enilive dato il numero (stringa, normalizzata)."""')
    L.append('    if numero is None:')
    L.append('        return None')
    L.append('    return CARTE_ENILIVE_BY_NUMERO.get(str(numero).strip())')
    L.append('')
    L.append('')
    L.append('def get_classificazione_carta_enilive(numero: str) -> Optional[str]:')
    ...
    L.append('    info = get_carta_enilive(numero)')
...
def main():
    ...
    print(f"== Generator carte_enilive_mapping ==")
