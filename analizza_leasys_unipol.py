...
for nome, search_key in fornitori:
    ...
    print(f"Fatture con OdA: {sum(oda_usage.values())}")
    ...
    print(f"\nUltime 20 fatture:")
    print(f"  {'Data':<12} {'Num':<18} {'Origin':<20} {'Importo':>10} Stato")
    for m in moves[-20:]:
        origin = (m.get('invoice_origin') or '')[:19]
        print(f"  {m.get('invoice_date', '?'):<12} "
              f"{(m.get('ref') or '?')[:17]:<18} "
              f"{origin:<20} "
    ...
    if total_with_oda > 0:
        ...
        if unique_odas == 1:
            ...
            print(f"-> MOLTI OdA diversi (fatturazione on-demand)")
    elif no_oda_count > 0:
        print(f"\n--- Pattern ---")
        print(f"-> {no_oda_count} fatture SENZA OdA collegato")